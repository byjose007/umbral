...
export const deviceProvisioning = pgTable(
  ...
  (table) => ({
    ...
    thumbprintIdx: uniqueIndex('idx_dev_prov_thumbprint').on(
      table.certificateThumbprint,
    ),
  }),
);
...
export const deviceHealthLogs = pgTable(
  'device_health_logs',
  {
    ...
    controllerId: varchar('controller_id', { length: 36 })
      .notNull()
      .references(() => controllers.id, { onDelete: 'cascade' }),
    appliedMatrixVersion: integer('applied_matrix_version')
      .notNull()
      .default(0),
    firmwareVersion: varchar('firmware_version', { length: 64 })
      .notNull()
      .default('1.0.0'),
    batteryStatus: varchar('battery_status', { length: 32 })
      .notNull()
      .default('ok'),
  ...
  (table) => ({
    controllerIdIdx: uniqueIndex('idx_health_logs_ctrl_id').on(
      table.controllerId,
    ),
  }),
);
